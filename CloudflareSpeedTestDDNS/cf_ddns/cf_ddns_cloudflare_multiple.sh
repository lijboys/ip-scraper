...
updateDNSRecords() {
  ...
  if [[ -f $csv_file ]]; then
    ...
    ip_count=0  # Counter for the number of IPs added for DNS update
    while [[ ${x} -lt ${num} ]] && [[ ${ip_count} -lt 5 ]]; do
      ipAddr=$(sed -n "$((x + 2))p" ./cf_ddns/result.csv | awk -F, '{print $1}')
      ipSpeed=$(sed -n "$((x + 2))p" ./cf_ddns/result.csv | awk -F, '{print $6}')
...
# 添加上传到 GitHub 的命令
echo "准备运行upload_to_github.py"
python3 /root/sp/cf_ddns/upload_to_github.py

if [[ $? -eq 0 ]]; then
  echo "upload_to_github.py 运行成功"
else
  echo "upload_to_github.py 运行失败"
fi
